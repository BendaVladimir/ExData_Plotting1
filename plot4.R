...
###################################################################
###  Saving plot to PNG image plot4.png
###################################################################
...
plot(chart_data$Global_active_power, type = "l", xaxt="none", xlab = "", ylab = "Global active power")
axis(side =1, at= c(1, nrow(chart_data)/2, nrow(chart_data)), labels = c("Thu", "Fri", "Sat"))

plot(chart_data$Voltage, type = "l", xaxt="none", xlab = "datetime", ylab = "Volatge")
axis(side =1, at= c(1, nrow(chart_data)/2, nrow(chart_data)), labels = c("Thu", "Fri", "Sat"))
...
axis(side =1, at= c(1, nrow(chart_data)/2, nrow(chart_data)), labels = c("Thu", "Fri", "Sat"))
legend("topright", legend = c("Sub metering 1", "Sub metering 2", "Sub metering 3"), col = c("black", "red", "blue"), lty = 1, box.lwd = 0, box.col = "white")

plot(chart_data$Global_reactive_power, type = "l", xaxt="none", xlab = "datetime", ylab = "Global reactive power")
axis(side =1, at= c(1, nrow(chart_data)/2, nrow(chart_data)), labels = c("Thu", "Fri", "Sat"))
dev.off()